...
The coefficient of each x is the expected difference in y (on the logarithmic scale) for each additional unit of the X. (Why is this? What is link function of the Poisson?) To interpret coefficients, we can exponentiate them and treat them as multiplicative effects. 
...
Let's also compare these models using a rootogram, you might have read about this in Ward! We're going to install a package here from a repo which makes it easy to do this. Rootograms help us understand the predictive accuracy of a model based on actual data. It will show us the expected counts given the model vs. the observed counts. It does so by square rooting the counts, which allows us to more easily examine departures from our expectations even at smaller frequencies. 
...
sims_mat <- mvrnorm(____, _____, ____)
dim(sims_mat)
...
Below, try using ggplot to plot this information we just generated. Follow these guidlines:
