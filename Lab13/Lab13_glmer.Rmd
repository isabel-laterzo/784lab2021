...
mod <- glmer(_________, data = _____, family = _______))

## present results
## note thsi is for an HTML
...
Now, we are going to use some new functions here form `plyr` - look into how `ldply()` and `ddply()` work, then try out the following. We just used `alply()` above, which is a related function. 
```{r}
...
## plot predicted probabilities using ggplot()
# put Q97 on the  x axis, your point ests on the y, and include upper and lower bounds
...
To make this more interesting, let's first make our country-level intercepts a function of a country-level variable: area (in square kilometers, `AREA_scaled`). 

So, below fit a similar model as before, but now include `AREA_scaled` as a predictor. 

```{r}
mod2 <- glmer(_________)
summary(mod2)
```
...
```{r}
## filter for malaqi, tanzania, and zambia
countries_in_pred <- c(___________)
area <- au %>% filter(_________)
```
...
Look up `bootMer` and run it on our mod2 with the function specified. Include 20 sims, set a seed, 8 ncpus, and call multicore parallel. What does it mean when we call `$t`?
```{r, eval=FALSE}
## Obtain approximate standard error 
## of random intercepts via parametric 
## bootstrap (this can take a few minutes)
ranef_sd_se <- bootMer(______, function(x)as.data.frame(VarCorr(x))$sdcor
                       ________)$t 
...
pred_Z <- as.matrix(model.matrix(_________) #Remove intercept

ranef_ab <- cbind(lme4::ranef(mod2)$COUNTRY, COUNTRY = rownames(lme4::ranef(mod2)$COUNTRY))

sub_ranef_ab <- merge(___, ____, all.x = TRUE) #merge randdom affect and the au data
...
mod3 <- glmer(__________, data = ___, family = _____)
...
## fit model
## nest region in country
## DV: Q52A
## IVS: Q4A, Q11A, Q11B, Q97

mod <- clmm(______, data = ___)
#doesn't work!
```
...
So, let's try again! Mutate those variables to factors, then re run the model.
```{r, results = 'asis'}
## convert outcome and country to factor
ab <- ab %>% mutate()

## fit model
mod4 <- clmm(________, data = __)
