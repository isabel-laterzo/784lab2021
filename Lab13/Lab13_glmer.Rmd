...
### Quantities Over Range of A Variable (What We Discussed in Lecture)
...
Now, we are going to use some new functions here form `plyr` - look into how `ldply()` and `ddply()` work, then try out the following:
```{r}
...
Look up `bootMer` and run it on our mod2 with the function specified. Include 20 sims, set a seed, 8 ncpus, and call multicore parallel.
```{r, eval=FALSE}
